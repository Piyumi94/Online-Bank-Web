...
public class UserService {
    ...
    public User createUser(User user) {
       ...
       // List<UserRepresentation> userRepresentations = keycloakUserService.readUserByEmail(user.getEmail());
		/*
		 * if (userRepresentations.size() > 0) { throw new
		...
		/*
		 * Integer userCreationResponse =
		 * keycloakUserService.createUser(userRepresentation);
		 * 
		 ...
		 * user.setIdentification(UUID.randomUUID().toString()); UserEntity save =
		 * userRepository.save(userMapper.convertToEntity(user)); return
		 * userMapper.convertToDto(save); }
		 */
	...
	/*
	 * public List<User> readUsers(Pageable pageable) { Page<UserEntity>
	 * allUsersInDb = userRepository.findAll(pageable); List<User> users =
	 * userMapper.convertToDtoList(allUsersInDb.getContent()); users.forEach(user ->
	 * { UserRepresentation userRepresentation =
	 * keycloakUserService.readUser(user.getAuthId()); user.setId(user.getId());
	 * user.setEmail(userRepresentation.getEmail());
	 * user.setIdentification(user.getIdentification()); }); return users; }
	 */
